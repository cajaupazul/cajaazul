...
export default function BouncingBalls() {
  ...
  useEffect(() => {
    ...
    const initialBalls = Array.from({ length: count }).map((_, i) => ({
      id: i,
      x: Math.random() * 100,
      y: Math.random() * 100,
      size: isMobile ? Math.random() * 150 + 80 : Math.random() * 300 + 100,
      duration: Math.random() * 20 + 20,
      delay: Math.random() * 5,
      blur: isMobile ? 'blur-2xl' : 'blur-3xl',
    }));
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      {balls.map((ball) => (
        <motion.div
          key={ball.id}
          className={`absolute rounded-full opacity-30 ${ball.blur} mix-blend-multiply filter`}
          style={{
