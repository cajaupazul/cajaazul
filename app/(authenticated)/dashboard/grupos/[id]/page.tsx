...
export default function GrupoDetailPage() {
  ...
  const fetchGrupoDetails = async () => {
    try {
      ...
      setGrupo(grupoData);
      setIsAdmin(grupoData.created_by === profile?.id);
      setEditData({ link_whatsapp: grupoData.link_whatsapp || '' });
      ...
      if (miembrosData && miembrosData.length > 0) {
        ...
        const miembrosConInfo = miembrosData.map(m => {
          const perfil = profilesData?.find(p => p.id === m.user_id);
          console.log('Miembro:', m.user_id, 'Perfil:', perfil);
          return {
            ...m,
            profile: perfil,
          };
        ...
        console.log('Miembros con info:', miembrosConInfo);
        setMiembros(miembrosConInfo);
      } else {
        setMiembros([]);
      }

      const isMemberCheck = miembrosData?.some(m => m.user_id === profile?.id);
  ...
  const handleUpdateWhatsapp = async () => {
    try {
      ...
      if (error) throw error;
      setGrupo({ ...grupo, link_whatsapp: editData.link_whatsapp });
