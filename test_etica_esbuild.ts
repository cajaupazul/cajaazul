...
writeFileSync('./apps/web/lib/pdf-test-input.txt', text);

import { parseLines } from './apps/web/lib/pdf-schedule-parser.ts';

const result = parseLines(text.split('\\n'));

console.log('\\n--- SECTION A ---');
console.log(JSON.stringify(result.filter(x => x.seccion === 'A'), null, 2));

console.log('\\n--- ALL SECTIONS ---');
console.log([...new Set(result.map(x => x.seccion))].sort());
